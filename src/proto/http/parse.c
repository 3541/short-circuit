...
static bool sc_http_request_first_line_parse(ScHttpRequest* req) {
    ...
    if (a3_buf_len(buf) >= SC_HTTP_REQUEST_LINE_MAX_LENGTH) {
        sc_http_response_error_prep_and_send(resp, SC_HTTP_STATUS_URI_TOO_LONG, SC_HTTP_CLOSE);
        return false;
    }
    ...
    case SC_HTTP_METHOD_INVALID:
        ...
        sc_http_response_error_prep_and_send(resp, SC_HTTP_STATUS_BAD_REQUEST, SC_HTTP_CLOSE);
        return false;
    case SC_HTTP_METHOD_UNKNOWN:
        A3_TRACE_F("Unimplemented HTTP method \"" A3_S_F "\".", A3_S_FORMAT(method));
        sc_http_response_error_prep_and_send(resp, SC_HTTP_STATUS_NOT_IMPLEMENTED, SC_HTTP_KEEP);
        return false;
    default:
        break;
    }
    ...
    if (conn->version == SC_HTTP_VERSION_UNKNOWN) {
        ...
        sc_http_response_error_prep_and_send(resp, SC_HTTP_STATUS_VERSION_NOT_SUPPORTED,
                                             SC_HTTP_KEEP);
        return false;
    }
...
static bool sc_http_request_headers_parse(ScHttpRequest* req) {
    ...
    if (req->transfer_encodings != SC_HTTP_TRANSFER_ENCODING_IDENTITY) {
        sc_http_response_error_prep_and_send(resp, SC_HTTP_STATUS_NOT_IMPLEMENTED, SC_HTTP_CLOSE);
        return false;
    }
    ...
    if (req->transfer_encodings == SC_HTTP_TRANSFER_ENCODING_IDENTITY) {
        ...
        if (req->content_length != SC_HTTP_CONTENT_LENGTH_UNSPECIFIED &&
            ...
            sc_http_response_error_prep_and_send(resp, SC_HTTP_STATUS_PAYLOAD_TOO_LARGE,
                                                 SC_HTTP_CLOSE);
            return false;
        }
